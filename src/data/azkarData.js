// بيانات الأذكار
export const AZKAR_DATA = {
  morning: [
    {
      id: 1,
      type: "morning",
      category: "quran",
      text: "بِسْمِ اللَّهِ الرَّحْمَٰنِ الرَّحِيمِ الْحَمْدُ لِلَّهِ رَبِّ الْعَالَمِينَ * الرَّحْمَٰنِ الرَّحِيمِ * مَالِكِ يَوْمِ الدِّينِ * إِيَّاكَ نَعْبُدُ وَإِيَّاكَ نَسْتَعِينُ * اهْدِنَا الصِّرَاطَ الْمُسْتَقِيمَ * صِرَاطَ الَّذِينَ أَنْعَمْتَ عَلَيْهِمْ غَيْرِ الْمَغْضُوبِ عَلَيْهِمْ وَلَا الضَّالِّينَ",
      repeat: 1,
      order: 1,
      source: "سورة الفاتحة",
      virtue: "والنبي صلى الله عليه وآله وسلم يقول  لجابر بن عبد الله رضي الله عنهما: «يَا جَابِرُ، أُخْبُرِكَ بِخَيْرِ سُورَةٍ نَزَلَتْ فِي الْقُرْآنِ؟» قَالَ: بَلَى يَا رَسُولَ اللهِ، قَالَ: «فَاتِحَةُ الْكِتَابِ»، قال راوي الحديث: وأحسبه قال: «فِيَها شِفَاءٌ مِنْ كُلِّ دَاءٍ» رواه البيهقي في شعب الإيمان.",
    },
    {
      id: 2,
      type: "morning",
      category: "quran",
      text: "اللَّهُ لَا إِلَٰهَ إِلَّا هُوَ الْحَيُّ الْقَيُّومُ ۚ لَا تَأْخُذُهُ سِنَةٌ وَلَا نَوْمٌ ۚ لَهُ مَا فِي السَّمَاوَاتِ وَمَا فِي الْأَرْضِ ۗ مَنْ ذَا الَّذِي يَشْفَعُ عِنْدَهُ إِلَّا بِإِذْنِهِ ۚ يَعْلَمُ مَا بَيْنَ أَيْدِيهِمْ وَمَا خَلْفَهُمْ ۖ وَلَا يُحِيطُونَ بِشَيْءٍ مِنْ عِلْمِهِ إِلَّا بِمَا شَاءَ ۚ وَسِعَ كُرْسِيُّهُ السَّمَاوَاتِ وَالْأَرْضَ ۖ وَلَا يَئُودُهُ حِفْظُهُمَا ۚ وَهُوَ الْعَلِيُّ الْعَظِيمُ",
      repeat: 1,
      order: 2,
      source: "آية الكرسي",
      virtue: "من قالها حين يصبح أجير من الجن حتى يمسى ومن قالها حين يمسى أجير من الجن حتى يصبح."
    },
    {
      id: 3,
      type: "morning",
      category: "dua",
      text: "أَصْـبَحْنا وَأَصْـبَحَ المُـلْكُ لله وَالحَمدُ لله ، لا إلهَ إلاّ اللّهُ وَحدَهُ لا شَريكَ لهُ، لهُ المُـلكُ ولهُ الحَمْـد، وهُوَ على كلّ شَيءٍ قدير ، رَبِّ أسْـأَلُـكَ خَـيرَ ما في هـذا اليوم وَخَـيرَ ما بَعْـدَه ، وَأَعـوذُ بِكَ مِنْ شَـرِّ ما في هـذا اليوم وَشَرِّ ما بَعْـدَه، رَبِّ أَعـوذُبِكَ مِنَ الْكَسَـلِ وَسـوءِ الْكِـبَر ، رَبِّ أَعوذُ بِكَ مِنْ عَـذابٍ في النّـارِ وَعَـذابٍ في القَـبْر.",
      repeat: 1,
      order: 3,
      source: "صحيح مسلم",
      virtue: "من جوامع أذكار الصباح دعاء «أصبحنا وأصبح الملك لله»، يستقبل به المسلم يومه بإقرار الملك لله وحمده. ثم يسأل ربه خير يومه ويستعيذ به من شره ومن الكسل وسوء الكبر وعذاب النار والقبر."
    },
    {
      id: 4,
      type: "morning",
      category: "dua",
      text: "اللَّهُمَّ بِكَ أَصْبَحْنَا، وَبِكَ أَمْسَيْنَا، وَبِكَ نَحْيَا، وَبِكَ نَمُوتُ، وَإِلَيْكَ النُّشُورُ",
      repeat: 1,
      order: 4,
      source: "الترمذي",
    },
    {
      id: 5,
      type: "morning",
      category: "tawheed",
      text: "أَصْبَحْنَا عَلَى فِطْرَةِ الْإِسْلَامِ، وَعَلَى كَلِمَةِ الْإِخْلَاصِ، وَعَلَى دِينِ نَبِيِّنَا مُحَمَّدٍ صَلَّى اللَّهُ عَلَيْهِ وَسَلَّمَ، وَعَلَى مِلَّةِ أَبِينَا إِبْرَاهِيمَ، حَنِيفًا مُسْلِمًا، وَمَا كَانَ مِنَ الْمُشْرِكِينَ",
      repeat: 1,
      order: 5,
      source: "أحمد",
    },
    {
      id: 6,
      type: "morning",
      category: "dua",
      text: "سُبْحَانَ اللَّهِ وَبِحَمْدِهِ، عَدَدَ خَلْقِهِ، وَرِضَا نَفْسِهِ، وَزِنَةَ عَرْشِهِ، وَمِدَادَ كَلِمَاتِهِ",
      repeat: 3,
      order: 6,
      source: "صحيح مسلم",
      virtue: " وعن جويرية - رضي الله عنها - أن النبي خرج من عندها بكرة حين صلى الصبح ، وهي في مسجدها ، ثم رجع بعد أن أضحى وهي جالسة ، قال : ما زلت على الحال التي فارقتك عليها ؟  قالت : نعم قال النبي - صلى الله عليه وسلم - :  لقد قلت بعدك أربع كلمات ثلاث مرات ، لو وزنت بما قلت منذ اليوم لوزنتهن : سبحان الله وبحمده عدد خلقه ، ورضاء نفسه ، وزنة عرشه ، ومداد كلماته    ( رواه مسلم ) ."
    },
    {
      id: 7,
      type: "morning",
      category: "dua",
      text: "اللَّهُمَّ إِنِّي أَصْبَحْتُ أُشْهِدُكَ، وَأُشْهِدُ حَمَلَةَ عَرْشِكَ، وَمَلَائِكَتَكَ، وَجَمِيعَ خَلْقِكَ، أَنَّكَ أَنْتَ اللَّهُ لَا إِلَهَ إِلَّا أَنْتَ وَحْدَكَ لَا شَرِيكَ لَكَ، وَأَنَّ مُحَمَّدًا عَبْدُكَ وَرَسُولُكَ",
      repeat: 4,
      order: 7,
      source: "أبو داود",
      virtue: "مروي من حديث أنس بن مالك -رضي الله عنه-، وله عنه طريقان:الأولى: أخرجها أبو داود، والترمذي، والضياء في المختارة، وغيرهم من طرقٍ عن بقية بن الوليد، قال: حدثني مسلم بن زياد. قال: سمعت أنس بن مالك -رضي الله عنه- يقول: إن رسول الله -صلى الله عليه وسلم- قال: من قال حين يصبح: اللهم أصبحنا نشهدك ونشهد حملة عرشك وملائكتك وجميع خلقك أنك أنت الله، لا إله إلا أنت وحدك لا شريك لك، وأن محمدًا عبدك ورسولك، إلا غفر الله له ما أصاب في يومه ذلك، وإن قالها حين يمسي غفر الله له ما أصاب في تلك الليلة من ذنب. واللفظ لـ الترمذي."
    },
    {
      id: 8,
      type: "morning",
      category: "dua",
      text: "اللَّهُمَّ مَا أَصْبَحَ بِي مِنْ نِعْمَةٍ أَوْ بِأَحَدٍ مِنْ خَلْقِكَ فَمِنْكَ وَحْدَكَ لَا شَرِيكَ لَكَ، فَلَكَ الْحَمْدُ وَلَكَ الشُّكْرُ",
      repeat: 1,
      order: 8,
      source: "أبو داود",
      virtue: "من قالها حين يصبح أدى شكر يومه."
    },
    {
      id: 9,
      type: "morning",
      category: "dua",
      text: "اللّهُـمَّ إِنِّـي أسْـأَلُـكَ العَـفْوَ وَالعـافِـيةَ في الدُّنْـيا وَالآخِـرَة ، اللّهُـمَّ إِنِّـي أسْـأَلُـكَ العَـفْوَ وَالعـافِـيةَ في ديني وَدُنْيَايَ وَأهْـلي وَمالـي ، اللّهُـمَّ اسْتُـرْ عـوْراتي وَآمِـنْ رَوْعاتـي ، اللّهُـمَّ احْفَظْـني مِن بَـينِ يَدَيَّ وَمِن خَلْفـي وَعَن يَمـيني وَعَن شِمـالي ، وَمِن فَوْقـي ، وَأَعوذُ بِعَظَمَـتِكَ أَن أُغْـتالَ مِن تَحْتـي.",
      repeat: 1,
      order: 9,
      source: "أبو داود",
      virtue: " استعاذة بالله وحراسة ربانية شاملة من أمامك ومن خلفك وعن يمينك وشمالك ومن فوقك، ومن الخسف أو الهلاك المفاجئ من تحتك (التي تعني الاغتيال من تحت الأرض أو السقوط المفاجئ)."
    },
    {
      id: 10,
      type: "morning",
      category: "dua",
      text: "حَسْبِيَ اللَّهُ لَا إِلَهَ إِلَّا هُوَ عَلَيْهِ تَوَكَّلْتُ وَهُوَ رَبُّ الْعَرْشِ الْعَظِيمِ",
      repeat: 7,
      order: 10,
      source: "أبو داود",
      virtue: "من قالها كفاه الله ما أهمه من أمر الدنيا والأخرة."
    },
    {
      id: 11,
      type: "morning",
      category: "tawheed",
      text: "اللّهُـمَّ عالِـمَ الغَـيْبِ وَالشّـهادَةِ فاطِـرَ السّماواتِ وَالأرْضِ رَبَّ كـلِّ شَـيءٍ وَمَلِيكَهُ ، أَشْهَـدُ أَنْ لا إِلـهَ إِلاّ أَنْت ، أَعوذُ بِكَ مِن شَـرِّ نَفْسـي وَمِن شَـرِّ الشَّيْـطانِ وَشِرْكِهِ ، وَأَنْ أَقْتَـرِفَ عَلـى نَفْسـي سوءاً أَوْ أَجُـرَّهُ إِلـى مُسْـلِم.",
      repeat: 1,
      order: 11,
      source: "الترمذى",
      virtue: "- أنَّ أبا بَكرٍ الصِّدِّيقَ رَضيَ اللهُ عنهما قال: يا رَسولَ اللهِ، مُرْني بكَلِماتٍ أقولُهُنَّ إذا أصبَحتُ، وإذا أمسَيتُ. قال: قُلِ: اللَّهمَّ فاطِرَ السَّمَواتِ والأرضِ، عالِمَ الغَيبِ والشَّهادةِ، رَبَّ كُلِّ شَيءٍ ومَليكَه، أشهَدُ أنْ لا إلهَ إلَّا أنتَ، أعوذُ بِكَ مِن شَرِّ نَفْسي ومِن شَرِّ الشَّيطانِ وشِركِه. قال: قُلْها إذا أصبَحتَ وإذا أمسَيتَ وإذا أخَذتَ مَضجَعَكَ."
    },
    {
      id: 12,
      type: "morning",
      category: "quran",
      text: "بِسْمِ اللهِ الرَّحْمنِ الرَّحِيمِ قُلۡ هُوَ ٱللَّهُ أَحَدٌ ﴿١﴾ ٱللَّهُ ٱلصَّمَدُ ﴿٢﴾ لَمۡ يَلِدۡ وَلَمۡ يُولَدۡ ﴿٣﴾ وَلَمۡ يَكُن لَّهُۥ كُفُوًا أَحَدُۢ ﴿٤﴾",
      repeat: 3,
      order: 12,
      source: "سورة الإخلاص",
      virtue: "من قالها حين يصبح وحين يمسى كفته من كل شىء (الإخلاص والمعوذتين)."
    },
    {
      id: 13,
      type: "morning",
      category: "quran",
      text: "بِسْمِ اللهِ الرَّحْمنِ الرَّحِيمِ قُلۡ أَعُوذُ بِرَبِّ ٱلۡفَلَقِ ﴿١﴾ مِن شَرِّ مَا خَلَقَ ﴿٢﴾ وَمِن شَرِّ غَاسِقٍ إِذَا وَقَبَ ﴿٣﴾ وَمِن شَرِّ ٱلنَّفَّٰثَٰتِ فِي ٱلۡعُقَدِ ﴿٤﴾ وَمِن شَرِّ حَاسِدٍ إِذَا حَسَدَ ﴿٥﴾",
      repeat: 3,
      order: 13,
      source: "البخاري",
      virtue: "من قالها حين يصبح وحين يمسى كفته من كل شىء (الإخلاص والمعوذتين)."
    },
    {
      id: 14,
      type: "morning",
      category: "quran",
      text: "بِسْمِ اللهِ الرَّحْمنِ الرَّحِيم\nقُلۡ أَعُوذُ بِرَبِّ ٱلنَّاسِ ﴿١﴾ مَلِكِ ٱلنَّاسِ ﴿٢﴾ إِلَٰهِ ٱلنَّاسِ ﴿٣﴾ مِن شَرِّ ٱلۡوَسۡوَاسِ ٱلۡخَنَّاسِ ﴿٤﴾ ٱلَّذِي يُوَسۡوِسُ فِي صُدُورِ ٱلنَّاسِ ﴿٥﴾ مِنَ ٱلۡجِنَّةِ وَٱلنَّاسِ ﴿٦﴾",
      repeat: 3,
      order: 14,
      source: "البخاري",
      virtue: "من قالها حين يصبح وحين يمسى كفته من كل شىء (الإخلاص والمعوذتين)."
    },
    {
      id: 15,
      type: "morning",
      category: "tasbih",
      text: "اللَّهُمَّ صَلِّ وَسَلِّمْ وَبَارِكْ على نَبِيِّنَا مُحمَّد",
      repeat: 10,
      order: 15,
      source: "صحيح مسلم",
      virtue:"من صلى على حين يصبح وحين يمسى ادركته شفاعتى يوم القيامة."
    },
    {
      id: 16,
      type: "morning",
      category: "tasbih",
      text: "أسْتَغْفِرُ اللهَ وَأتُوبُ إلَيْهِ",
      repeat: 100,
      order: 16,
      source: "صحيح مسلم",
      virtue: "مائة حسنة، ومُحيت عنه مائة سيئة، وكانت له حرزاً من الشيطان حتى يمسى."
    },
    {
      id: 17,
      type: "morning",
      category: "tasbih",
      text: "سُبْحَانَ اللَّهِ وَبِحَمْدِهِ",
      repeat: 100,
      order: 17,
      source: "صحيح مسلم",
      virtue: "حُطَّتْ خَطَايَاهُ وَإِنْ كَانَتْ مِثْلَ زَبَدِ الْبَحْرِ. لَمْ يَأْتِ أَحَدٌ يَوْمَ الْقِيَامَةِ بِأَفْضَلَ مِمَّا جَاءَ بِهِ إِلَّا أَحَدٌ قَالَ مِثْلَ مَا قَالَ أَوْ زَادَ عَلَيْهِ."
    },
    {
      id: 18,
      type: "morning",
      category: "tasbih",
      text: "لَا إلَه إلّا اللهُ وَحْدَهُ لَا شَرِيكَ لَهُ، لَهُ الْمُلْكُ وَلَهُ الْحَمْدُ وَهُوَ عَلَى كُلِّ شَيْءٍ قَدِيرٌ.",
      repeat: 100,
      order: 18,
      source: "صحيح مسلم",
      virtue:"كانت له عدل عشر رقاب، وكتبت له مئة حسنة، ومحيت عنه مئة سيئة، وكانت له حرزا من الشيطان."
    },
  ],
  evening: [
    {
      id: 101,
      type: "evening",
      category: "quran",
      text: "بِسْمِ اللَّهِ الرَّحْمَٰنِ الرَّحِيمِ الْحَمْدُ لِلَّهِ رَبِّ الْعَالَمِينَ * الرَّحْمَٰنِ الرَّحِيمِ * مَالِكِ يَوْمِ الدِّينِ * إِيَّاكَ نَعْبُدُ وَإِيَّاكَ نَسْتَعِينُ * اهْدِنَا الصِّرَاطَ الْمُسْتَقِيمَ * صِرَاطَ الَّذِينَ أَنْعَمْتَ عَلَيْهِمْ غَيْرِ الْمَغْضُوبِ عَلَيْهِمْ وَلَا الضَّالِّينَ",
      repeat: 1,
      order: 1,
      source: "سورة الفاتحة",
      virtue: "والنبي صلى الله عليه وآله وسلم يقول  لجابر بن عبد الله رضي الله عنهما: «يَا جَابِرُ، أُخْبُرِكَ بِخَيْرِ سُورَةٍ نَزَلَتْ فِي الْقُرْآنِ؟» قَالَ: بَلَى يَا رَسُولَ اللهِ، قَالَ: «فَاتِحَةُ الْكِتَابِ»، قال راوي الحديث: وأحسبه قال: «فِيَها شِفَاءٌ مِنْ كُلِّ دَاءٍ» رواه البيهقي في شعب الإيمان."
    },
    {
      id: 102,
      type: "evening",
      category: "quran",
      text: "اللَّهُ لَا إِلَٰهَ إِلَّا هُوَ الْحَيُّ الْقَيُّومُ ۚ لَا تَأْخُذُهُ سِنَةٌ وَلَا نَوْمٌ ۚ لَهُ مَا فِي السَّمَاوَاتِ وَمَا فِي الْأَرْضِ ۗ مَنْ ذَا الَّذِي يَشْفَعُ عِنْدَهُ إِلَّا بِإِذْنِهِ ۚ يَعْلَمُ مَا بَيْنَ أَيْدِيهِمْ وَمَا خَلْفَهُمْ ۖ وَلَا يُحِيطُونَ بِشَيْءٍ مِنْ عِلْمِهِ إِلَّا بِمَا شَاءَ ۚ وَسِعَ كُرْسِيُّهُ السَّمَاوَاتِ وَالْأَرْضَ ۖ وَلَا يَئُودُهُ حِفْظُهُمَا ۚ وَهُوَ الْعَلِيُّ الْعَظِيمُ",
      repeat: 1,
      order: 2,
      source: "آية الكرسي",
      virtue: "من قالها حين يصبح أجير من الجن حتى يمسى ومن قالها حين يمسى أجير من الجن حتى يصبح."
    },
    {
      id: 103,
      type: "morning",
      category: "quran",
      text: `بِسْمِ اللهِ الرَّحْمنِ الرَّحِيمِ قُلۡ هُوَ ٱللَّهُ أَحَدٌ ﴿١﴾ ٱللَّهُ ٱلصَّمَدُ ﴿٢﴾ لَمۡ يَلِدۡ وَلَمۡ يُولَدۡ ﴿٣﴾ وَلَمۡ يَكُن لَّهُۥ كُفُوًا أَحَدُۢ ﴿٤﴾`,
      repeat: 3,
      order: 3,
      source: "سورة الإخلاص",
      virtue: "من قالها حين يصبح وحين يمسى كفته من كل شىء (الإخلاص والمعوذتين)."
    },
    {
      id: 104,
      type: "morning",
      category: "quran",
      text: `بِسْمِ اللهِ الرَّحْمنِ الرَّحِيمِ قُلۡ أَعُوذُ بِرَبِّ ٱلۡفَلَقِ ﴿١﴾ مِن شَرِّ مَا خَلَقَ ﴿٢﴾ وَمِن شَرِّ غَاسِقٍ إِذَا وَقَبَ ﴿٣﴾ وَمِن شَرِّ ٱلنَّفَّٰثَٰتِ فِي ٱلۡعُقَدِ ﴿٤﴾ وَمِن شَرِّ حَاسِدٍ إِذَا حَسَدَ ﴿٥﴾`,
      repeat: 3,
      order: 4,
      source: "البخاري",
      virtue: "من قالها حين يصبح وحين يمسى كفته من كل شىء (الإخلاص والمعوذتين)."
    },
    {
      id: 105,
      type: "morning",
      category: "quran",
      text: "بِسْمِ اللهِ الرَّحْمنِ الرَّحِيم قُلۡ أَعُوذُ بِرَبِّ ٱلنَّاسِ ﴿١﴾ مَلِكِ ٱلنَّاسِ ﴿٢﴾ إِلَٰهِ ٱلنَّاسِ ﴿٣﴾ مِن شَرِّ ٱلۡوَسۡوَاسِ ٱلۡخَنَّاسِ ﴿٤﴾ ٱلَّذِي يُوَسۡوِسُ فِي صُدُورِ ٱلنَّاسِ ﴿٥﴾ مِنَ ٱلۡجِنَّةِ وَٱلنَّاسِ ﴿٦﴾",
      repeat: 3,
      order: 5,
      source: "البخاري",
      virtue: "من قالها حين يصبح وحين يمسى كفته من كل شىء (الإخلاص والمعوذتين)."
    },
    {
      id: 106,
      type: "evening",
      category: "dua",
      text: "أَمْسَيْنَا وَأَمْسَى الْمُلْكُ لِلَّهِ، وَالْحَمْدُ لِلَّهِ، لَا إِلَهَ إِلَّا اللَّهُ وَحْدَهُ لَا شَرِيكَ لَهُ، لَهُ الْمُلْكُ وَلَهُ الْحَمْدُ وَهُوَ عَلَى كُلِّ شَيْءٍ قَدِيرٌ، رَبِّ أَسْأَلُكَ خَيْرَ مَا فِي هَذِهِ اللَّيْلَةِ وَخَيْرَ مَا بَعْدَهَا، وَأَعُوذُ بِكَ مِنْ شَرِّ مَا فِي هَذِهِ اللَّيْلَةِ وَشَرِّ مَا بَعْدَهَا رَبِّ أَعُوذُ بِكَ مِنَ الْكَسَلِ وَسُوءِ الْكِبَرِ، رَبِّ أَعُوذُ بِكَ مِنْ عَذَابٍ فِي النَّارِ وَعَذَابٍ فِي الْقَبْرِ.",
      repeat: 1,
      order: 6,
      source: "صحيح مسلم",
      virtue: ": يتضمن هذا الدعاء تفويض الأمور لله عز وجل، وطلب خير ما في الليلة وما يأتي بعدها من أوقات، والاستعاذة به سبحانه وتعالى من شرورها."
    },
    {
      id: 107,
      type: "evening",
      category: "dua",
      text: "اللّهـمَّ أَنْتَ رَبِّـي لا إلهَ إلاّ أَنْتَ ، خَلَقْتَنـي وَأَنا عَبْـدُك ، وَأَنا عَلـى عَهْـدِكَ وَوَعْـدِكَ ما اسْتَـطَعْـت ، أَعـوذُبِكَ مِنْ شَـرِّ ما صَنَـعْت ، أَبـوءُ لَـكَ بِنِعْـمَتِـكَ عَلَـيَّ وَأَبـوءُ بِذَنْـبي فَاغْفـِرْ لي فَإِنَّـهُ لا يَغْـفِرُ الذُّنـوبَ إِلاّ أَنْتَ .",
      repeat: 1,
      order: 7,
      source: "الترمذي",
      virtue:"من قالها موقنا بها حين يمسى ومات من ليلته دخل الجنة وكذلك حين يصبح."
    },
    {
      id: 108,
      type: "evening",
      category: "tawheed",
      text: "أَمْسَيْنَا عَلَى فِطْرَةِ الْإِسْلَامِ، وَعَلَى كَلِمَةِ الْإِخْلَاصِ، وَعَلَى دِينِ نَبِيِّنَا مُحَمَّدٍ صَلَّى اللَّهُ عَلَيْهِ وَسَلَّمَ، وَعَلَى مِلَّةِ أَبِينَا إِبْرَاهِيمَ، حَنِيفًا مُسْلِمًا، وَمَا كَانَ مِنَ الْمُشْرِكِينَ",
      repeat: 1,
      order: 8,
      source: "أحمد",
    },
    {
      id: 109,
      type: "evening",
      category: "dua",
      text: "سُبْحَانَ اللَّهِ وَبِحَمْدِهِ، عَدَدَ خَلْقِهِ، وَرِضَا نَفْسِهِ، وَزِنَةَ عَرْشِهِ، وَمِدَادَ كَلِمَاتِهِ",
      repeat: 3,
      order: 9,
      source: "صحيح مسلم",
      virtue: " وعن جويرية - رضي الله عنها - أن النبي خرج من عندها بكرة حين صلى الصبح ، وهي في مسجدها ، ثم رجع بعد أن أضحى وهي جالسة ، قال : ما زلت على الحال التي فارقتك عليها ؟  قالت : نعم قال النبي - صلى الله عليه وسلم - :  لقد قلت بعدك أربع كلمات ثلاث مرات ، لو وزنت بما قلت منذ اليوم لوزنتهن : سبحان الله وبحمده عدد خلقه ، ورضاء نفسه ، وزنة عرشه ، ومداد كلماته    ( رواه مسلم ) ."
    },
    {
      id: 110,
      type: "evening",
      category: "dua",
      text: "اللَّهُمَّ إِنِّي أَمْسَيْتُ أُشْهِدُكَ، وَأُشْهِدُ حَمَلَةَ عَرْشِكَ، وَمَلَائِكَتَكَ، وَجَمِيعَ خَلْقِكَ، أَنَّكَ أَنْتَ اللَّهُ لَا إِلَهَ إِلَّا أَنْتَ وَحْدَكَ لَا شَرِيكَ لَكَ، وَأَنَّ مُحَمَّدًا عَبْدُكَ وَرَسُولُكَ",
      repeat: 4,
      order: 10,
      source: "أبو داود",
      virtue: "مروي من حديث أنس بن مالك -رضي الله عنه-، وله عنه طريقان:الأولى: أخرجها أبو داود، والترمذي، والضياء في المختارة، وغيرهم من طرقٍ عن بقية بن الوليد، قال: حدثني مسلم بن زياد. قال: سمعت أنس بن مالك -رضي الله عنه- يقول: إن رسول الله -صلى الله عليه وسلم- قال: من قال حين يصبح: اللهم أصبحنا نشهدك ونشهد حملة عرشك وملائكتك وجميع خلقك أنك أنت الله، لا إله إلا أنت وحدك لا شريك لك، وأن محمدًا عبدك ورسولك، إلا غفر الله له ما أصاب في يومه ذلك، وإن قالها حين يمسي غفر الله له ما أصاب في تلك الليلة من ذنب. واللفظ لـ الترمذي."
    },
    {
      id: 111,
      type: "evening",
      category: "dua",
      text: "اللَّهُمَّ مَا أَمْسَى بِي مِنْ نِعْمَةٍ أَوْ بِأَحَدٍ مِنْ خَلْقِكَ فَمِنْكَ وَحْدَكَ لَا شَرِيكَ لَكَ، فَلَكَ الْحَمْدُ وَلَكَ الشُّكْرُ",
      repeat: 1,
      order: 11,
      source: "أبو داود",
      virtue: "من قالها حين يمسى أدى شكر يومه."
    },
    {
      id: 112,
      type: "evening",
      category: "dua",
      text: "اللّهُـمَّ إِنِّـي أسْـأَلُـكَ العَـفْوَ وَالعـافِـيةَ في الدُّنْـيا وَالآخِـرَة ، اللّهُـمَّ إِنِّـي أسْـأَلُـكَ العَـفْوَ وَالعـافِـيةَ في ديني وَدُنْيَايَ وَأهْـلي وَمالـي ، اللّهُـمَّ اسْتُـرْ عـوْراتي وَآمِـنْ رَوْعاتـي ، اللّهُـمَّ احْفَظْـني مِن بَـينِ يَدَيَّ وَمِن خَلْفـي وَعَن يَمـيني وَعَن شِمـالي ، وَمِن فَوْقـي ، وَأَعـوذُ بِعَظَمَـتِكَ أَن أُغْـتالَ مِن تَحْتـي.",
      repeat: 3,
      order: 12,
      source: "أبو داود",
      virtue: " استعاذة بالله وحراسة ربانية شاملة من أمامك ومن خلفك وعن يمينك وشمالك ومن فوقك، ومن الخسف أو الهلاك المفاجئ من تحتك (التي تعني الاغتيال من تحت الأرض أو السقوط المفاجئ)."
    },
    {
      id: 113,
      type: "evening",
      category: "dua",
      text: "حَسْبِيَ اللَّهُ لَا إِلَهَ إِلَّا هُوَ عَلَيْهِ تَوَكَّلْتُ وَهُوَ رَبُّ الْعَرْشِ الْعَظِيمِ",
      repeat: 7,
      order: 13,
      source: "أبو داود",
      virtue: "من قالها كفاه الله ما أهمه من أمر الدنيا والأخرة."
    },
    {
      id: 114,
      type: "evening",
      category: "tawheed",
      text: "اللّهُـمَّ عالِـمَ الغَـيْبِ وَالشّـهادَةِ فاطِـرَ السّماواتِ وَالأرْضِ رَبَّ كـلِّ شَـيءٍ وَمَلِيكَهُ ، أَشْهَـدُ أَنْ لا إِلـهَ إِلاّ أَنْت ، أَعـوذُ بِكَ مِن شَـرِّ نَفْسـي وَمِن شَـرِّ الشَّيْـطانِ وَشِرْكِهِ ، وَأَنْ أَقْتَـرِفَ عَلـى نَفْسـي سوءاً أَوْ أَجُـرَّهُ إِلـى مُسْـلِم.",
      repeat: 10,
      order: 14,
      source: "البخاري",
      virtue: "- أنَّ أبا بَكرٍ الصِّدِّيقَ رَضيَ اللهُ عنهما قال: يا رَسولَ اللهِ، مُرْني بكَلِماتٍ أقولُهُنَّ إذا أصبَحتُ، وإذا أمسَيتُ. قال: قُلِ: اللَّهمَّ فاطِرَ السَّمَواتِ والأرضِ، عالِمَ الغَيبِ والشَّهادةِ، رَبَّ كُلِّ شَيءٍ ومَليكَه، أشهَدُ أنْ لا إلهَ إلَّا أنتَ، أعوذُ بِكَ مِن شَرِّ نَفْسي ومِن شَرِّ الشَّيطانِ وشِركِه. قال: قُلْها إذا أصبَحتَ وإذا أمسَيتَ وإذا أخَذتَ مَضجَعَكَ."
    },
    {
      id: 115,
      type: "morning",
      category: "tasbih",
      text: "اللَّهُمَّ صَلِّ وَسَلِّمْ وَبَارِكْ على نَبِيِّنَا مُحمَّد",
      repeat: 10,
      order: 15,
      source: "صحيح مسلم",
      virtue:"من صلى على حين يصبح وحين يمسى ادركته شفاعتى يوم القيامة."
    },
    {
      id: 116,
      type: "morning",
      category: "tasbih",
      text: "أسْتَغْفِرُ اللهَ وَأتُوبُ إلَيْهِ",
      repeat: 100,
      order: 16,
      source: "صحيح مسلم",
      virtue: "مائة حسنة، ومُحيت عنه مائة سيئة، وكانت له حرزاً من الشيطان حتى يمسى."
    },
    {
      id: 117,
      type: "evening",
      category: "tasbih",
      text: "سُبْحَانَ اللَّهِ وَبِحَمْدِهِ",
      repeat: 100,
      order: 17,
      source: "صحيح مسلم",
      virtue: "حُطَّتْ خَطَايَاهُ وَإِنْ كَانَتْ مِثْلَ زَبَدِ الْبَحْرِ. لَمْ يَأْتِ أَحَدٌ يَوْمَ الْقِيَامَةِ بِأَفْضَلَ مِمَّا جَاءَ بِهِ إِلَّا أَحَدٌ قَالَ مِثْلَ مَا قَالَ أَوْ زَادَ عَلَيْهِ."
    },
    {
      id: 118,
      type: "evening",
      category: "tasbih",
      text: "لَا إلَه إلّا اللهُ وَحْدَهُ لَا شَرِيكَ لَهُ، لَهُ الْمُلْكُ وَلَهُ الْحَمْدُ وَهُوَ عَلَى كُلِّ شَيْءٍ قَدِيرٌ.",
      repeat: 100,
      order: 18,
      source: "صحيح مسلم",
      virtue: "كانت له عدل عشر رقاب، وكتبت له مئة حسنة، ومحيت عنه مئة سيئة، وكانت له حرزا من الشيطان."
    }
  ]
};